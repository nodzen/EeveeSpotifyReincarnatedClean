...
//
// ── Token availability ───────────────────────────────────────────────────────
// SpotifyAccessTokenStore.value is captured lazily from Spotify's outgoing requests.
...
// We replicate that by polling SpotifyAccessTokenStore.value for up to 5 seconds before
...
class SpicyLyricsRepository: LyricsRepository {
    ...
    // MARK: - Token wait
    //
    // Poll for SpotifyAccessTokenStore.value up to `timeout` seconds.
